...
@section('content')
    ...
    activeTabIndex: {{ $type == 'main' ? 0 : 1 }}
}">
				<div class="justify-center my-6 font-bold tabs">
					<template x-for="(tab, index) in tabs" :key="index">
						<a
							:href="index == 0 ? '{{ route('master.expense-category') }}' : '{{ route('master.expense-category', 'sub-category') }}'"
				...
				<div class="pt-4 my-6 card">
					<div x-show="activeTabIndex == 0">
						<x-toggle-record-field>
							<livewire:records.add-category-record type='3'>
						</x-toggle-record-field>
						<hr>
						<div class="py-4">
							<livewire:tables.master.category-table category_type='3' />
						</div>
					</div>
